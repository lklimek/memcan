...
- [Ollama](https://ollama.com/) — LLM inference (embeddings are handled in-process by fastembed on the server). A GPU is strongly recommended for acceptable performance with the default model (`lklimek/gemma4-text:26b-a4b-it-qat`, ~16GB VRAM, projector-free build with full GPU residency). On an 8GB card, use `qwen3.5:9b` instead — see [`docs/memcan-model-guide.html`](docs/memcan-model-guide.html).
...
docker compose exec ollama ollama pull lklimek/gemma4-text:26b-a4b-it-qat
...
| `LLM_MODEL` | `lklimek/gemma4-text:26b-a4b-it-qat` | Ollama model name — bare (no prefix) by default; an `ollama::` prefix (e.g. `ollama::lklimek/gemma4-text:26b-a4b-it-qat`) is also accepted and stripped automatically. Needs ~16GB VRAM (projector-free build, full GPU residency on a 16GB card); use `qwen3.5:9b` (~6.6GB) on smaller cards. |
