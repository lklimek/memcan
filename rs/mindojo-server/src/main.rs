//! MindOJO — unified server binary with MCP transport + admin subcommands.

use std::path::PathBuf;

use clap::{Parser, Subcommand};
use mindojo_core::error::MindojoError;
...
#[command(
    name = "mindojo",
    about = "MindOJO — persistent memory for Claude Code",
    version = env!("MINDOJO_VERSION"),
)]
struct Cli {
    #[command(subcommand)]
...
pub struct ServeArgs {
    ...
    /// Listen address (overrides MINDOJO_LISTEN env var).
    #[arg(long)]
    ...
    match cli.command {
        ...
        Some(Command::DownloadModel(_args)) => {
            let (settings, _embedder) =
                mindojo_core::init::MindojoContext::init_settings_and_embedder()?;
            println!(
        ...
        Some(Command::Completions(args)) => {
            ...
            clap_complete::generate(args.shell, &mut cmd, "mindojo", &mut std::io::stdout());
            Ok(())
        }
    }
}
