//! Import triaged findings into vector store (moved from `memcan-import-triaged`).
...
use memcan_core::embed::FastEmbedProvider;
...
pub async fn run(args: &ImportTriagedArgs) -> MemcanResult<()> {
    ...
    let triage = report.triage.as_ref().ok_or_else(|| {
        MemcanError::Other("Report has no triage decisions. Run triage-findings first.".into())
    })?;
