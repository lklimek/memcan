...
impl FastEmbedProvider {
    ...
    pub fn new(model_name: EmbeddingModel, dims: usize) -> Result<Self> {
        ...
        let model = TextEmbedding::try_new(opts).map_err(|e| MemcanError::Embedding {
...
impl EmbeddingProvider for FastEmbedProvider {
    async fn embed(&self, texts: &[String]) -> Result<Vec<Vec<f32>>> {
        ...
        tokio::task::spawn_blocking(move || {
            let mut guard = model.lock().map_err(|e| MemcanError::Embedding {
                context: "model lock poisoned".into(),
                detail: e.to_string(),
            })?;
            guard.embed(docs, None).map_err(|e| MemcanError::Embedding {
                context: "fastembed embed failed".into(),
                detail: e.to_string(),
            })
        })
        .await
        .map_err(|e| MemcanError::Embedding {
