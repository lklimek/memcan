...
class TestFindEnvFile:
    ...
    def test_finds_first_existing(self, tmp_path):
        from mindajo_mcp.config import _find_env_file
...
class TestDefaultCandidates:
    ...
    def test_config_dir_is_first_candidate(self):
        from mindajo_mcp.config import _CONFIG_DIR

        # Config dir should be platform-appropriate and contain "mindajo"
        assert "mindajo" in str(_CONFIG_DIR)
    ...
    def test_cwd_env_used_when_config_dir_missing(self, tmp_path, monkeypatch):
        ...
        # Simulate: config dir doesn't have .env, only CWD does
