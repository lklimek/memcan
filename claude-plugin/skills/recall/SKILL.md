...
2. **Search both scopes** -- run `search_memories` twice: once with `project` set to git remote origin repo name (e.g., `memcan` not `memcan-2`), once without (global). Use `limit=5` each.
