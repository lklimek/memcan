...
3. **Count before** — `get_memories(project=<if scoped>)`, note the count.
4. **Save** — `add_memory(content=<text>, project=<if scoped>, metadata={"type": "<lesson|decision|preference>", ...})`. Or `update_memory` if updating.
5. **Count after** — `get_memories(...)` again, note the new count.
